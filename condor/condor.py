...
class condor_object:
    ...
    def initial_community(self, method = "LCS", project=False):
        ...
        if project:
            with Timer("Initial community structure with projection:"):
                ...
                if method=="LCS":
                    vc = Graph.community_multilevel(projected_graph,weights="weight")
                if method=="LEC":
                    vc = Graph.community_leading_eigenvector(projected_graph,weights="weight")
                if method=="FG":
                    vc = Graph.community_fastgreedy(projected_graph,weights="weight").as_clustering()
                if method=="LDN":
                    vc = Graph.community_leiden(projected_graph,weights="weight").as_clustering()
            ...
            with Timer("Initial community structure without projection:"):
                if method=="LCS":
                    vc = Graph.community_multilevel(self.graph,weights="weight")
                if method=="LEC":
                    vc = Graph.community_leading_eigenvector(self.graph,weights="weight")
                if method=="FG":
                    vc = Graph.community_fastgreedy(self.graph,weights="weight").as_clustering()
                if method=="LDN":
                    vc = Graph.community_leiden(self.graph,weights="weight").as_clustering()
    ...
    def brim(self,deltaQmin="def",c="def",resolution=1):
        ...
        #Default deltaQmin.import numpy as np
import pandas as pd
from igraph import *
from .timer import Timer
...
class condor_object:
    ...
    def initial_community(self, method = "LCS", project=False):
        ...
        print(weights_id,type(weights_id))
        if project:
            with Timer("Initial community structure with projection:"):
